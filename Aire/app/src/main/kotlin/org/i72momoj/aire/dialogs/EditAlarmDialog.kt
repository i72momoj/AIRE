...
class EditAlarmDialog(
    ...
    init {
        ...
        binding.apply {
            ...
            if(layout and CONFIRMAR_CREACION != 0) {
            ...
            else if(layout and CONFIRMAR_MODIFICACION != 0) {
                val params = mensaje.layoutParams
                params.height = ViewGroup.LayoutParams.WRAP_CONTENT

                mensaje.layoutParams = params
                mensaje.text = "¿Desea confirmar la modificación?"
            }
            else if(layout and CONFIRMAR_BORRADO != 0) {
                val params = mensaje.layoutParams
                params.height = ViewGroup.LayoutParams.WRAP_CONTENT

                mensaje.layoutParams = params
                mensaje.text = "¿Desea confirmar la eliminación?"
            }
            else
                mensaje.height = 0
            ...
            if(layout and CONFIRMAR_VOZ == 0) {
                val layoutParams = visualizer.layoutParams
                layoutParams.height = 0
                visualizer.layoutParams = layoutParams
            }

            // En caso de que estemos confirmando una eliminación, no podremos modificar los datos
            if (layout and CONFIRMAR_BORRADO == 0) {
                    ...
                    activity.setupDialogStuff(binding.root, this) { alertDialog ->
                        alertDialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                            ...
                            activity.handleFullScreenNotificationsPermission { granted ->
                                if (granted) {
                                    if(layout and CONFIRMAR_CREACION != 0 || layout and CONFIRMAR_MODIFICACION != 0) {
                                        if (alarm.id == 0) {
