...
class AlarmaAsistente(
    ...
    companion object {
        ...
        fun copiar(alarma: Alarm): Alarm {
            val alarma = Alarm(
                id = alarma.id,
                timeInMinutes = alarma.timeInMinutes,
                days = alarma.days,
                isEnabled = alarma.isEnabled,
                vibrate = alarma.vibrate,
                soundTitle = alarma.soundTitle,
                soundUri = alarma.soundUri,
                label = alarma.label,
                oneShot = alarma.oneShot
            )
        ...
        fun desdeDatosModelo(datosExtraidos: ArrayList<Pair<String, String>>?, alarma: Alarm?, extraer: Int = DATOS_ALARMA): Alarm? {
            var hora = ""
            var minutos = ""
            var mtn: Int = -1
            ...
            for(datos in datosExtraidos!!) {
                ...
                if(extraer == DATOS_ALARMA) {
                    when (datos.first) {
                        ...
                        "B-MTN" -> {
                            mtn = if(mtn > -1) mtn else when (datos.second) {
                                "mañana" -> MTN_MANIANA
                                "tarde" -> MTN_TARDE
                                "noche" -> MTN_NOCHE
                                else -> -1
                    ...
                    when (datos.first) {
                        "B-ACTH" -> hora += datos.second
                        "I-ACTH" -> hora += " ${datos.second}"
                        "B-ACTM" -> minutos += datos.second
                        "I-ACTM" -> minutos += " ${datos.second}"
                        "B-MTN" -> {
